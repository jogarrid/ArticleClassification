...
Classification_model: contains various .py (SVM.py, neuralNetwork.py, naiveBayes.py, catBoost.py, clustering.py) files, each of them estimates the test error using crossvalidation for a different machine learning algorithm to predict whether someone is fired or not based on the vectorial representation of what they have written about (along with the other parameters already described, which can be excluded by executing with the argument -i False). For this, it loads data_preprocessed.csv, the file created by executing loadData.py
